...
- Windows 控制台采用宽屏双栏布局、自绘标题栏和统一的深色状态卡片，快捷动作、悬浮菜单与手机直连状态更易查看

### 修复

- 修复高刷新率桌面上悬浮球拖动和松手吸附仍按固定约 60Hz 更新，造成明显拖影与不跟手的问题；移动节拍现与 Windows DWM 合成同步
- 修复按刷新帧数计算 UU 静止收尾导致 120Hz/144Hz 下超时提前的问题，改为按真实 400ms 时间判断

### 变更

- 悬浮菜单重定位快捷键从鼠标前进侧键改为鼠标中键，避免 GameViewer 将侧键改写为普通左键后无法安全区分
- Windows 控制台默认尺寸调整为 1180×760，最小尺寸调整为 960×640，并保留关闭到托盘语义

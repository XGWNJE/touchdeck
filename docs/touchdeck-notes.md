...
## 2026-08-21 重定位触发键改为鼠标中键

- 当前 GameViewer 输入链路把鼠标前进键和正常左键都上报为相同的 `WM_LBUTTONDOWN`，连 Windows 低层钩子的 `flags` 与 `dwExtraInfo` 也完全一致，TouchDeck 无法安全区分。
- 重定位菜单触发键因此改为鼠标中键（`VK_MBUTTON`）；原有淡出、传送、淡入和菜单展开语义不变，左键选择与长按拖动不变。

## 2026-08-14 鼠标侧键展开、菜单涌现动画与叠影排查（v0.3.2）

### 2026-08-14 修复完成（自动验证 + Owner 目检通过）

- 删除 Windows 菜单窗口的独立 `#hub` 球芯，菜单中心保持透明，直接透出唯一的真实悬浮球；从结构上消除两个透明窗口各绘一颗球造成的 DPI/亚像素叠影。
- 悬浮球 CSS 按 Chromium 内容区短边保持严格正圆；菜单内环间距读取实际可见球径，不再用窗口外框单边推算。
- 侧键传送由固定 60ms 定时改为 `opacity transitionend` 回执：淡出确认后才 `SetWindowPos`，淡入确认后才展开菜单；主进程校验发件渲染器和请求 ID，并阻止并发传送。
- 新增 bubble renderer ready 握手，避免冷启动首条 fade IPC 早于监听安装；菜单窗口按球所在显示器创建，并把全局锚点换算为菜单局部坐标。
- 自动验证：`npm test`（23 个主测试 + 4 个信令测试）、`npm run typecheck`、`npm run build`、`git diff --check` 全通过；150% DPI 下用 `desktopCapturer` 截取侧键传送后的稳定帧，只见单一半透明球轮廓，日志无 fade ACK 超时；快速双按未产生交错菜单。Owner 在可见预览中确认本次一次性修复成功。

### 已验证（自验 + 用户验收）

- 侧键（XButton2 前进键）展开/收起菜单：球淡出→传送到鼠标位置→淡入，菜单以球实际中心为锚点展开，球保留在新位置；菜单展开期间再按侧键，鼠标在球上则收起、移走则传送球到新位置重新展开。（自验：`typecheck`/`build` 通过；Owner 可见预览验收通过。）
...
- **最终根因与修复（2026-08-14）**：问题不是单一尺寸误差，而是球窗口的半透明 `#ball` 与菜单窗口的不透明 `#hub` 重复绘制同一视觉元素；DPI 取整、亚像素和两套抗锯齿使跨透明窗口无法逐像素重合。侧键传送又用固定 60ms 猜测绘制完成，放大了旧位置残帧。最终删除 `#hub`、按内容区短边保持真实球为正圆，并以 `transitionend` 回执驱动淡出→移动→淡入→展开，Owner 验收确认残影与双层球均消失。

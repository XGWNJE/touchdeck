...
contextBridge.exposeInMainWorld("touchdeck", {
  ...
  // 侧键传送：渲染端必须在 opacity 过渡真正结束后回执，主进程才移动/展开菜单。
  onBubbleFade: (cb: (visible: boolean, requestId: string) => void) => ipcRenderer.on("bubble-fade", (_e: IpcRendererEvent, visible: boolean, requestId: string) => cb(visible, requestId)),
  bubbleFadeComplete: (requestId: string) => ipcRenderer.send("bubble-fade-complete", requestId),

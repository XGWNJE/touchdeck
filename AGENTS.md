...
v0.2.2「远程触控指令可靠性闭环」和 v0.2.3「安全配对」已完成受控验证，v0.3.3「桌面控制台与高刷新率悬浮菜单体验修复」已完成。当前唯一产品主线仍是 v0.3.0「单一黄金工作流」的真实使用验证：

- v0.2.2/v0.2.3 已完成真实手机和平板的首次配对、跨信令重启续连、双设备定向 ACK 和反馈验收。
- 陌生用户无需修改 JSON，即可在 10 分钟内完成安装、配对、发送和中断。
- 当前只打磨一个 GUI 编程 Agent 默认预设，动作最终收敛到 8–12 个。
- v0.3.0 锁定动作语义而不锁死键位或触发模式；所有默认动作可在 Windows Host 控制台选择已知预设，或自定义单组组合键及“单击触发/按住保持”，无需修改 JSON。
- 1.0 前不实现旧用户配置迁移；测试版配置结构变化时允许重置，并要求用户重新选择语音方案和快捷键。
...
- 新增第二个或第三个场景预设。
- 开发通用 GUI 配置编辑器、自由拖拽画布或任意组件系统。唯一例外是 v0.3.0 既定动作的快捷键预设选择与自定义组合键设置。
...
2. 按钮和界面参数配置驱动。`touchdeck.config.json` 是默认动作事实来源，资源来自 `themes/`、`layouts/`、`icons/`；v0.3.0 用户快捷键绑定入口必须在 Windows Host 控制台，并写入 Electron `userData`，不得要求普通用户改 JSON，也不得把用户按钮写死在业务代码中。
...
9. `hold` 动作是从 Android 按住到松开的 `keyDown/keyUp` 会话，不是延迟后执行一次普通快捷键。`end`、触摸取消、断线、Host 停止和看门狗超时必须无条件释放按键；保持期间不得让其他宏与尚未释放的修饰键交错。
...
- 信令断线、WebRTC 半开和 Host 闪断已有重连机制；Host 在线时房间滚动续期，不再由固定 30 分钟 TTL 中断连续使用。
...
- Host 指纹用于人工核对主机身份。Host/设备凭据与活动配对密钥只以哈希写入受限状态仓库；普通关闭只结束会话，“忘记全部设备”才撤销登记。状态文件缺失或损坏时生产服务拒绝启动。
...
| 变更类型 | 必须验证 |
|---|---|
| 协议/ACK | 自动测试覆盖正常、拦截、失败、超时、断线、重复请求、非法消息和队列溢出；按住动作额外覆盖 begin/end 幂等、忙碌拒绝和异常释放 |
...
| 快捷键自定义 | 预设下拉、自定义组合键、单击/按住触发模式、重复/冲突提示、单项/全部恢复，以及 Windows/Android 实际键位同步；1.0 前不验收旧配置迁移 |
| 按住保持 | Codex 听写达到防误触门槛后开始、松开结束；滑出、`ACTION_CANCEL`、双端进程退出、断线和 60 秒超时均释放，结束后系统不存在卡住的修饰键 |
...
- `docs/workflow-preset-design.md`：v0.3.0 动作预设的设计假设、候选分级和 Owner 验证问题。
